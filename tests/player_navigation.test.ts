...
describe('React player navigation and state restoration', () => {
  ...
  it('uses React-owned navigation, drawers and native dialog semantics', () => {
    ...
    expect(views).toContain('<PlayerFooterBar variant="immersive" isActive={footerHost === \'immersive\'} />')
    expect(views).not.toContain('{open && <PlayerFooterBar variant="immersive" />}')
    expect(footer).toContain('id={footerId}')
    expect(footer).toContain('player-footer-immersive-hidden')
    expect(shell).toContain('isActive={!hidden}')
    expect(footer).not.toContain('react-immersive-utility-actions')
    expect(views).not.toContain('react-immersive-header-label')
    expect(shell).not.toContain('react-sidebar-brand')
    ...
    expect(css).toMatch(/\.react-immersive-lyrics-list > button:focus:not\(:focus-visible\)\s*\{\s*outline: 0 !important;\s*box-shadow: none !important;/)
    expect(css).toMatch(/\.react-immersive-lyrics-list > button:focus-visible\s*\{\s*outline: 0 !important;\s*box-shadow: inset 0 0 0 1px var\(--app-accent\) !important;/)
    ...
    expect(shell).toContain('immersiveFooterHost')
    ...
    expect(shell).toContain('<PlayerFooter hidden={immersiveFooterHost !== \'normal\'} />')
  ...
  it('keeps the normal footer order and exposes the portal song action flow', () => {
    ...
    const views = read('frontend/player/src/react/views.tsx')
    const normalFooter = footer.slice(footer.indexOf('<div className="react-footer-controls">'))
    const controls = normalFooter.slice(0, normalFooter.indexOf('<div className="react-footer-song-section">'))
    const footerActions = normalFooter.slice(normalFooter.indexOf('<div className="react-footer-actions">'))
    expect(normalFooter.indexOf('react-footer-controls')).toBeLessThan(normalFooter.indexOf('react-footer-song-section'))
    expect(normalFooter.indexOf('react-footer-song-section')).toBeLessThan(normalFooter.indexOf('react-footer-actions'))
    expect(controls).toContain('react-like-button')
    expect(footerActions).not.toContain('react-like-button')
    expect(normalFooter.indexOf('react-song-menu-button')).toBeLessThan(normalFooter.indexOf('react-mode-button'))
    expect(normalFooter.indexOf('打开播放队列')).toBeLessThan(normalFooter.indexOf('<VolumeControl'))
    expect(footer).toContain('react-volume-popover')
    expect(footer).toContain('react-volume-value')
    ...
    expect(css).toContain('.react-volume-control.is-open')
    expect(css).toContain('.react-footer-controls .react-like-button span')
    expect(css).toContain('grid-template-columns: auto minmax(0, 1fr);')
    expect(css).toContain('.react-player-footer#player-footer .react-footer-actions { display: none; }')
    expect(css).toContain('inline-size: min(9.875rem, calc(100vw - 1rem));')
  ...
  it('keeps homepage shortcut motion restrained and persists custom playlist icons', () => {
    ...
    const shell = read('frontend/player/src/react/shell.tsx')
    const css = read('frontend/styles/player.css')
    const shortcutStyles = css.slice(css.indexOf('.react-home-shortcut {'), css.indexOf('.react-home-section {'))
    ...
    expect(libraryViews).toContain('react-home-shortcut-surface')
    expect(libraryViews).toContain('react-home-playlist-surface')
    expect(libraryViews).toContain('react-home-playlist-arrow')
    expect(libraryViews).toContain('react-artwork-cover ${kind === \'artist\' ? \'is-artist\' : \'\'}')
    expect(views).toContain('type="radio"')
    expect(views).toContain('aria-label={option.label}')
    expect(views).not.toContain('<legend>选择歌单氛围</legend>')
    ...
    expect(shortcutStyles).toContain('contain: paint;')
    expect(shortcutStyles).toContain('--react-home-shortcut-drift-from:')
    expect(shortcutStyles).toContain('--react-home-shortcut-drift-to:')
    expect(shortcutStyles).not.toContain('filter: blur(')
    for (const duration of ['14s', '17s', '21s', '24s']) expect(css).toContain(`--react-home-shortcut-duration: ${duration};`)
    ...
    expect(css).toContain('.react-home-playlist:hover .react-home-playlist-arrow')
    expect(css).toContain('.react-playlist-icon-option.is-selected .react-playlist-icon-option-visual')
    expect(css).toContain('overflow: hidden;')
    expect(css).toContain('background: var(--app-surface) !important;')
  ...
  it('renders one visible heading for playlist pages while retaining the browser title', () => {
    ...
    expect(views).not.toContain('<h2>歌曲</h2>{batchMode && selectedSongs.size > 0 && <p>已选择 {selectedSongs.size} 首</p>}')
    expect(views).toContain('react-header-selection-count')
  ...
  it('uses the reference artwork lyrics layout without the retired vinyl markup', () => {
    ...
    expect(views).toContain('<PlayerFooterBar variant="immersive" isActive={footerHost === \'immersive\'} />')
    expect(footer).toContain('react-immersive-detail-controls')
    expect(footer).toContain('react-immersive-detail-transport')
    ...
    expect(css).toContain('@keyframes react-immersive-detail-control-enter')
  ...
  it('uses the shared settings account and keeps source management in the admin app', () => {
    ...
    expect(views).not.toContain('自定义音源由管理员在管理后台配置')
    expect(views).not.toContain('AboutView')
