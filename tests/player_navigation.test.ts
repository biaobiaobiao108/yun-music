...
describe('Player Navigation and State Restoration Safety', () => {
    ...
    it('player footer stays unified and floating across player views', () => {
        ...
        expect(visualizer).toContain('播放栏是独立的固定悬浮层，主页面内容不再为它预留底部空间');
        ...
        expect(css).toContain('backdrop-filter: blur(20px) saturate(145%) contrast(1.02) !important;');
        expect(css).toContain('transform: translate3d(0, 0, 0);');
        expect(css).toContain('.player-scroll-surface');
