...
describe('React admin frontend', () => {
  ...
  it('uses React composition and Zustand state without a window app bridge', () => {
    ...
    for (const view of ['DashboardView', 'UsersView', 'StorageView', 'DataView', 'ConfigView', 'LogsView', 'SnapshotsView', 'CustomSourcesView', 'AboutView']) expect(sources).toContain(view)
  ...
  it('uses quiet auto-dismiss toasts and keeps source management out of the player', () => {
    const adminEntry = read('frontend/admin/src/react/index.tsx')
    const adminCss = read('frontend/styles/admin.css')
    const playerEntry = read('frontend/player/src/react/components.tsx')
    const playerViews = read('frontend/player/src/react/views.tsx')
    const playerCss = read('frontend/styles/player.css')
    expect(adminEntry).toContain('toastPhase')
    expect(adminEntry).not.toContain('toast-close')
    ...
    expect(playerViews).toContain('自定义音源由管理员在管理后台配置')
