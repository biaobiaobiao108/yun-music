...
- **现代化界面**: 采用清爽的冷灰背景、翡翠绿强调色和响应式布局。
- **多源搜索**: 支持聚合搜索各大音乐平台的资源。
- **歌单与收藏**: 数据保存在 Web Server 实例中，也支持本地音乐和缓存。
- **PWA 支持**: 支持安装为桌面/手机独立应用，离线也能秒开。
- **自定义源**: 自定义音源由管理员在管理后台配置，播放器只读取当前账户已分配且启用的音源。
...
支持 HttpOnly、SameSite 会话 Cookie，浏览器不会保存密码或旧版 Token。
...
2. **音频数据**: 本项目本身没有获取某个音频数据的能力，所使用的在线音频数据来源来自管理员配置并分配给当前账户的自定义源返回的在线链接。本项目无法校验其准确性，使用过程中可能会出现播放异常。
