...
播放器底部栏和沉浸式歌词页是两个独立的 React 挂载位置，但都复用 `PlayerFooterBar` 的同一套胶囊结构与尺寸 token。普通底部栏负责歌曲菜单、播放控制和抽屉入口；歌词页只隐藏三个点菜单，保留评论、下载、睡眠定时器等直接操作，不再使用黑胶唱片或独立 footer 布局。歌词页关闭时先以短暂透明度动画退出，再关闭原生 `dialog`，避免播放栏、封面和进度条重新计算造成跳动。

## 路由与历史

React 继续兼容以下 hash 地址：

`#search`、`#songlist`、`#leaderboard`、`#favorites`、`#localmusic`、`#settings`、`#about`，以及首页、最近、专辑、歌手和音乐库入口。旧的 `#genres` 仍可被解析以兼容历史链接，但不再出现在侧栏，也不作为新入口。
...
后台列表采用“侧边列表导航 + 右侧密集表格”的结构。歌曲实体显示会优先使用归一化后的封面、歌手、专辑、ID、时长和格式；没有封面时使用本地云音占位图。表格滚动容器使用稳定行高、`content-visibility` 和 `scrollbar-gutter`，按钮统一为紧凑语义按钮，避免旧模板的巨大按钮和低对比度文字覆盖 React 组件。移动端列表导航会变为横向滚动条，歌曲表保留横向滚动而不裁切信息。
...
- 普通底部栏在歌词页打开时卸载，歌词页内部只挂载一个共享结构的播放栏；两者使用相同的封面尺寸、进度条和响应式间距，避免进入歌词页时发生尺寸跳变。歌词页关闭时优先恢复打开它的封面按钮，找不到原节点时回退到主内容区域。
