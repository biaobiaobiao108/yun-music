...
export function SnapshotsView() {
  ...
  return <ViewFrame title="快照管理" subtitle="备份、恢复与删除用户歌单快照"><Panel title="快照列表" actions={<div className="admin-react-toolbar admin-snapshot-toolbar"><label className="admin-react-inline-label admin-snapshot-user-field">用户<SelectMenu label="快照用户" value={selectedUser} options={[{ value: '', label: '请选择' }, ...users.filter(user => user.name !== '_open').map(user => ({ value: user.name, label: user.name }))]} onChange={setSelectedUser} /></label><form onSubmit={upload} className="admin-react-upload-form admin-snapshot-upload"><label className="admin-react-file-picker"><Icon name="file-lines" /><span title={snapshotFile?.name}>{snapshotFile?.name ?? '选择快照'}</span><input className="admin-react-file-input" type="file" accept="application/json,.json" onChange={event => setSnapshotFile(event.target.files?.[0] ?? null)} aria-label="选择快照文件" /></label><Button variant="primary" type="submit" disabled={!snapshotFile || !selectedUser || Boolean(busyAction)}>上传</Button></form><Button className="admin-snapshot-action" onClick={requestDownloadBackup} disabled={Boolean(busyAction)}><Icon name="download" />下载备份</Button><form onSubmit={requestRestoreBackup} className="admin-react-upload-form admin-snapshot-upload"><label className="admin-react-file-picker"><Icon name="file-zipper" /><span title={backupFile?.name}>{backupFile?.name ?? '选择备份 ZIP'}</span><input className="admin-react-file-input" type="file" accept=".zip,application/zip" onChange={event => setBackupFile(event.target.files?.[0] ?? null)} aria-label="选择全量备份文件" /></label><Button variant="danger" type="submit" disabled={!backupFile || Boolean(busyAction)}>恢复</Button></form><Button className="admin-snapshot-action" onClick={requestRestart} disabled={Boolean(busyAction)}><Icon name="rotate" />重启服务</Button></div>}>{snapshots.length ? <div className="admin-react-table-wrap"><table className="admin-react-table"><caption className="sr-only">快照列表</caption><thead><tr><th scope="col">快照</th><th scope="col">大小</th><th scope="col">时间</th><th scope="col">操作</th></tr></thead><tbody>{snapshots.map((snapshot, index) => { const id = String(snapshot.id ?? snapshot.name ?? index); return <tr key={id}><th scope="row">{id}</th><td>{formatBytes(snapshot.size)}</td><td>{formatDate(snapshot.time)}</td><td className="admin-react-row-actions"><Button onClick={() => void downloadSnapshot(id)} disabled={Boolean(busyAction)}><Icon name="download" />下载</Button><Button onClick={() => requestRestoreSnapshot(id)} disabled={Boolean(busyAction)}><Icon name="rotate" />恢复</Button><Button variant="danger" onClick={() => requestDeleteSnapshot(id)} disabled={Boolean(busyAction)}><Icon name="trash" />删除</Button></td></tr> })}</tbody></table></div> : <Empty label={selectedUser ? '暂无快照' : '请选择用户'} />}</Panel><ConfirmDialog open={Boolean(confirmation)} title={confirmation?.title ?? ''} message={confirmation?.message ?? ''} confirmLabel={confirmation?.confirmLabel} onClose={() => setConfirmation(null)} onConfirm={() => confirmation?.onConfirm()} /></ViewFrame>
}
