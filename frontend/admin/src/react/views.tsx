...
export function StorageView() {
  ...
  return <ViewFrame title="存储管理" subtitle="查看缓存与下载音乐，执行安全清理"><Panel title="文件存储" actions={<div className="admin-react-toolbar"><label className="admin-react-inline-label">用户<SelectMenu label="存储用户" value={selectedUser} options={[{ value: '', label: '全部用户' }, { value: '_open', label: '公开空间' }, ...users.filter(user => user.name !== '_open').map(user => ({ value: user.name, label: user.name }))]} onChange={setSelectedUser} /></label><Button onClick={() => void loadView('storage')}><Icon name="rotate" />刷新</Button></div>}>
