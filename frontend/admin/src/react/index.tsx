...
function AdminShell() {
  ...
  return <div className="admin-react-shell"><div className={`mobile-sidebar-overlay ${mobileOpen ? 'active' : 'hidden'}`} onClick={() => setMobileOpen(false)} aria-hidden="true" /><aside className={`sidebar glass ${mobileOpen ? 'active' : ''}`} aria-label="管理导航"><div className="sidebar-header"><div className="sidebar-brand"><img src="/assets/yun-yin.png" className="logo-icon" width="42" height="42" alt="云音图标" /><h2>云音</h2></div><button className="admin-react-icon-button admin-react-mobile-close" aria-label="关闭导航菜单" onClick={() => setMobileOpen(false)}><Icon name="xmark" /></button></div><nav className="sidebar-nav"><p className="admin-react-nav-label">管理控制台</p>{NAV_ITEMS.map(item => <button key={item.id} type="button" className={`nav-item ${view === item.id ? 'active' : ''}`} aria-current={view === item.id ? 'page' : undefined} onClick={() => selectView(item.id)}><Icon name={item.icon} /><span>{item.label}</span></button>)}<a className="nav-item" href={String(config['player.path'] || '/music')}><Icon name="music" /><span>打开播放器</span></a></nav><div className="sidebar-footer"><Button onClick={() => void signOut()} className="btn-logout"><Icon name="right-from-bracket" />退出登录</Button></div></aside><div className="admin-react-main"><header className="admin-react-topbar"><button type="button" className="admin-react-menu-button" aria-label="打开导航菜单" aria-expanded={mobileOpen} onClick={() => setMobileOpen(true)}><Icon name="bars" /></button><div><p className="admin-react-eyebrow">云音管理后台</p><h1 id="page-title">{title}</h1></div><div className="admin-react-top-actions">{installPrompt && <Button onClick={() => void install()}><Icon name="download" />安装应用</Button>}<Button variant="secondary" onClick={() => void signOut()}><Icon name="right-from-bracket" />退出</Button></div></header><main id="admin-main" className="admin-react-content" tabIndex={-1}>{viewFor(view)}</main></div>{toast && <div className="admin-react-toast" role="status">{toast}</div>}</div>
}
