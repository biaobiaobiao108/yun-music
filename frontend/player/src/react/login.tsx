...
function LoginPage() {
  ...
  useEffect(() => { applyThemePreferences(readThemePreferences()); void Promise.all([playerApi.config(), playerApi.verify()]).then(([config, session]) => { if (!config['player.enableAuth'] || session.valid) window.location.replace('./'); else { setEnabled(true); setReady(true) } }).catch(() => { setError('初始化检查失败，请刷新页面重试'); setReady(true) }) }, [])
