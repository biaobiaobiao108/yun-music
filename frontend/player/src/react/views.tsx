...
function UserPlaylistView({ listId, list, loading, error }: { listId: string; list?: UserPlaylist; loading: boolean; error: string }) {
  ...
  if (!list) return <ViewFrame title="歌单" subtitle="正在恢复歌单内容"><section className="react-content-card t-bg-panel">{loading ? <Loading label="正在加载歌单…" /> : <div className="react-empty"><Icon name="list" /><p>歌单不存在或已被删除</p><Button variant="primary" onClick={() => openFavoriteList('love')}>返回我喜欢的音乐</Button></div>}</section></ViewFrame>
...
function CustomSourcesContent() {
  ...
  return <section className="react-content-card t-bg-panel react-custom-sources"><div className="react-section-heading"><div><p className="react-eyebrow">SOURCE PROVIDERS</p><h2>自定义音源</h2><p>管理公开与当前账户可用的第三方音源脚本</p></div><Button onClick={() => void load()} disabled={loading}><Icon name="rotate" />刷新</Button></div>{error && <p className="react-error" role="alert">{error}</p>}<div className="react-custom-source-list">{loading ? <Loading /> : sources.length ? sources.map(source => <article className={`react-custom-source-card ${source.enabled ? 'is-enabled' : ''}`} key={`${source.owner}-${source.id}`}><div className="react-custom-source-card-head"><span className="react-custom-source-icon"><Icon name="plug" /></span><div><strong title={String(source.name || source.id)}>{String(source.name || source.id)}</strong><small>{source.owner === 'open' || source.isPublic ? '公开音源' : '私有音源'}{source.version ? ` · v${String(source.version)}` : ''}</small></div><span className={`react-source-status ${source.enabled ? 'is-enabled' : ''}`}>{source.enabled ? '已启用' : '已停用'}</span></div>{source.error && <p className="react-custom-source-error">{String(source.error)}</p>}<div className="react-custom-source-card-actions"><Button onClick={() => void toggle(source)}>{source.enabled ? '停用' : '启用'}</Button>{source.owner !== 'open' && <Button variant="danger" onClick={() => setConfirmation(source)}>删除</Button>}</div></article>) : <p className="react-empty-text">暂无自定义音源</p>}</div><div className="react-source-forms"><form onSubmit={importSource}><label htmlFor="custom-source-url">从 URL 导入</label><div><input id="custom-source-url" type="url" value={url} onChange={event => setUrl(event.target.value)} placeholder="https://example.com/source.js" required /><Button variant="primary" type="submit">导入</Button></div></form><form onSubmit={uploadSource}><label htmlFor="custom-source-file">上传脚本</label><div><input id="custom-source-file" type="file" accept=".js,.json,application/javascript,application/json" onChange={event => setFile(event.target.files?.[0] ?? null)} /><Button type="submit" disabled={!file}>上传</Button></div></form></div><Modal open={Boolean(confirmation)} title="删除自定义源" onClose={() => setConfirmation(null)}><p>确定删除“{String(confirmation?.name || confirmation?.id || '')}”吗？</p><div className="react-dialog-actions"><Button onClick={() => setConfirmation(null)}>取消</Button><Button variant="primary" onClick={() => void remove()}>删除</Button></div></Modal></section>
}

function AudioEffectsPanel() {
  const settings = useSettingsStore(state => state.settings)
  const setSetting = useSettingsStore(state => state.setSetting)
  return <section className="react-content-card t-bg-panel react-audio-settings"><h2>音效处理</h2><div className="react-settings-form"><label className="react-switch-row"><input type="checkbox" checked={Boolean(settings.enableSoundEffects)} onChange={event => setSetting('enableSoundEffects', event.target.checked)} /><span>启用浏览器音效处理</span></label><div className="react-setting-field"><span>音效预设</span><SelectMenu label="音效预设" value={String(settings.soundEffectsPreset ?? 'flat')} options={[{ value: 'flat', label: '均衡' }, { value: 'vocal', label: '人声' }, { value: 'bass', label: '低音增强' }, { value: 'focus', label: '清晰度' }]} onChange={value => setSetting('soundEffectsPreset', value)} /></div><label>音效增益<input type="range" min="0.5" max="1.5" step="0.05" value={Number(settings.soundEffectsGain ?? 1)} onChange={event => setSetting('soundEffectsGain', Number(event.target.value))} /><output>{Number(settings.soundEffectsGain ?? 1).toFixed(2)}×</output></label><p className="react-setting-hint">播放器不再显示底部频谱；不支持 Web Audio 的浏览器会自动回退到原生播放。</p></div></section>
}
...
export function AddToListDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <Modal open={open} title="添加到歌单" onClose={onClose}><div className="react-add-to-list-dialog">{song && <p className="react-add-to-list-song"><SafeImage src={songImage(song)} width="44" height="44" alt="" /><span><strong>{songTitle(song)}</strong><small>{songArtist(song)}</small></span></p>}<div className="react-add-to-list-options">{lists.map(list => { const included = Boolean(song && list.songs.some(item => sameSong(item, song))); return <button type="button" key={list.id} className={`react-add-to-list-option ${included ? 'is-included' : ''}`} aria-pressed={included} onClick={() => void toggleList(list)}><span><Icon name={list.id === 'love' ? 'heart' : 'music'} /><strong>{list.name}</strong><small>{list.songs.length} 首歌曲</small></span><Icon name={included ? 'check' : 'plus'} /></button> })}<Button type="button" className="react-add-to-list-create" onClick={() => setDialog('createList')}><Icon name="plus" />新建歌单</Button></div>{!song && <p className="react-empty-text">当前没有正在播放的歌曲</p>}</div></Modal>
}
