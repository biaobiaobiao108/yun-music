...
export const useLibraryStore = create<LibraryState>((set, get) => ({
  ...
  renameList: async (listId, name) => { const data = get().data; const userList = (data.userList ?? []).map(list => list.id === listId ? { ...list, name } : list); await playerApi.saveListData({ ...data, userList }); await get().hydrate() },
  deleteList: async (listId) => { const data = get().data; await playerApi.saveListData({ ...data, userList: (data.userList ?? []).filter(list => list.id !== listId) }); await get().hydrate() },
}))
