...
function SongListGrid() {
  ...
  useEffect(() => {
    ...
    return <ViewFrame title="歌单广场"><section className="react-toolbar-card t-bg-panel"><div className="react-toolbar-field"><span>音源</span><SelectMenu label="音源" value={source} options={[{ value: 'wy', label: '网易云' }, { value: 'tx', label: 'QQ音乐' }]} onChange={value => { setSource(value); setCategory(''); setPage(1) }} /></div><div className="react-toolbar-field"><span>分类</span><SelectMenu label="分类" value={category} options={[{ value: '', label: '全部' }, ...categories.map(item => ({ value: item.id, label: item.name }))]} onChange={value => { setCategory(value); setPage(1) }} /></div></section><section className="react-content-card t-bg-panel">{loading ? <Loading /> : error ? <p className="react-error" role="alert">{error}</p> : songs.length ? <div className="react-playlist-grid">{songs.map((item, index) => { const id = listItemId(item, index); const name = String(item.name ?? item.title ?? item.dissname ?? '未命名歌单'); const sourceName = String(item.source || source); return <article className="react-playlist-card" key={`${sourceName}:${id}:${index}`}><button type="button" onClick={() => setDetail({ page: 'songlist-detail', kind: 'playlist', id, source: sourceName, name, image: listItemImage(item) })}><SafeImage src={listItemImage(item)} width="180" height="180" loading="lazy" alt={`${name}封面`} /><strong>{name}</strong><small>{String(item.creator ?? item.author ?? '平台歌单')} · {String(item.songCount ?? item.trackCount ?? item.total ?? '歌曲')}</small></button></article> })}</div> : <div className="react-empty"><Icon name="list" /><p>暂无歌单内容</p></div>}<div className="react-pagination react-pagination-bottom"><button type="button" aria-label="上一页" disabled={page <= 1 || loading} onClick={() => setPage(value => value - 1)}><Icon name="chevron-left" /></button><span>第 {page} 页</span><button type="button" aria-label="下一页" disabled={loading || songs.length < 20} onClick={() => setPage(value => value + 1)}><Icon name="chevron-right" /></button></div></section></ViewFrame>
}
...
export function LeaderboardView() {
  ...
  return <ViewFrame title="排行榜"><section className="react-toolbar-card t-bg-panel"><div className="react-toolbar-field"><span>音源</span><SelectMenu label="音源" value={source} options={[{ value: 'wy', label: '网易云' }, { value: 'tx', label: 'QQ音乐' }]} onChange={value => { setSource(value); setPage(1) }} /></div><div className="react-toolbar-field"><span>榜单</span><SelectMenu label="榜单" value={selected} disabled={!boards.length} options={boardOptions.length ? boardOptions : [{ value: '', label: '暂无榜单', disabled: true }]} onChange={value => { setSelected(value); setPage(1) }} /></div></section><section className="react-content-card t-bg-panel">{error && <p className="react-error" role="alert">{error}</p>}{loading ? <Loading /> : <SongList songs={songs} empty="暂无排行榜歌曲" />}<div className="react-pagination react-pagination-bottom"><button type="button" aria-label="上一页" disabled={page <= 1 || loading} onClick={() => setPage(value => value - 1)}><Icon name="chevron-left" /></button><span>第 {page} 页</span><button type="button" aria-label="下一页" disabled={loading || songs.length < 20} onClick={() => setPage(value => value + 1)}><Icon name="chevron-right" /></button></div></section></ViewFrame>
}
...
export function LocalMusicView() {
  ...
  const cacheSong = (item: CacheItem): Song => {
    ...
    return {
      ...
      singer: item.singer || songArtist(metadata),
      albumName: item.albumName || songAlbum(metadata),
      source: item.source || metadata.source,
      img: image,
      size: item.size ?? songSizeValue(metadata),
  ...
  const cacheDuration = (item: CacheItem, song: Song): string => {
    const value = item.duration ?? item.interval ?? songDurationValue(song)
