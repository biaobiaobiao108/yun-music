...
import { useLibraryStore, useMediaLibraryStore, usePlaybackStore, usePlayerUiStore, useRecentStore } from './store'
...
function imageOf(song: Song): string {
...
function entityId(song: Song): string {
  return String(song.albumId ?? song.albumid ?? song.artistId ?? song.artistid ?? song.id ?? song.songmid ?? songKey(song))
}

function detailFor(song: Song, kind: 'artist' | 'album'): PlayerDetail {
  return { page: 'search-detail', kind, id: entityId(song), source: sourceOf(song), name: String(kind === 'artist' ? song.singer || song.artist || song.name : song.album || song.name || '专辑'), image: imageOf(song) }
}
...
export function HomeView() {
  ...
  const playlistCount = library.userList?.length ?? 0
  ...
  const userLists = (library.userList ?? []).slice(0, 6)
  return <ViewFrame title="首页" subtitle="把喜欢的音乐，放在触手可及的地方"><section className="react-home-page">
    <div className="react-home-shortcuts">
      <Shortcut icon="heart" title="我的收藏" subtitle={`${library.loveList?.length ?? 0} 首歌曲`} className="is-love" onClick={() => openFavoriteList('love')} />
    ...
    <section className="react-home-section"><div className="react-home-section-heading"><div><p className="react-eyebrow">你的收藏</p><h2>我的歌单</h2></div><button type="button" className="react-text-button" onClick={() => setTab('favorites')}>管理歌单 <Icon name="arrow-right" /></button></div>{userLists.length ? <div className="react-playlist-grid react-home-playlists">{userLists.map((list, index) => { const first = list.list?.[0]; return <button type="button" className="react-home-playlist" key={`${list.id}-${index}`} onClick={() => openFavoriteList(String(list.id))}><SafeImage src={songImage(first)} width="112" height="112" loading="lazy" alt="" /><span><strong>{list.name}</strong><small>{list.list?.length ?? 0} 首歌曲</small></span><Icon name="arrow-right" /></button> })}</div> : <div className="react-home-empty"><Icon name="list" /><p>还没有自定义歌单</p><button type="button" className="react-text-button" onClick={() => setTab('favorites')}>创建歌单</button></div>}</section>
  </section></ViewFrame>
}
...
export function GenresView() {
  ...
  useEffect(() => {
    ...
    void playerApi.songList(source, selected, 'hot', page, controller.signal).then(payload => { if (!controller.signal.aborted) setSongs(listOf(payload)) }).catch(cause => { if (!controller.signal.aborted) { setSongs([]); setError(cause instanceof Error ? cause.message : '风格内容加载失败') } }).finally(() => { if (!controller.signal.aborted) setLoading(false) })
